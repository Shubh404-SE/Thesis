...
class DatasetProcessor:
    ...
    def process_patient(self, patient_path: str, output_patient_path: str) -> bool:
        ...
        try:
            ...
            for mri_type in self.mri_types:
                ...
                # Load DICOM series
                sitk_img = self.dicom_loader.load_dicom_series(input_mri_path)
                if sitk_img is None:
                    continue

                # Process image
                skull_stripped_sitk = self.image_processor.skull_strip(sitk_img)
                normalized_sitk = self.image_processor.normalize(skull_stripped_sitk)

                # Save result
                final_image_path = os.path.join(output_mri_path, "preprocessed.png")
                self.image_processor.save_preprocessed_image(normalized_sitk, final_image_path)
    ...
    def run_preprocessing(self) -> None:
        """Runs preprocessing on the dataset with progress tracking."""
        ...
        for split in ["train", "test"]:
            ...
            patient_folders = sorted(os.listdir(split_path))[:self.max_patients]
            total_patients = len(patient_folders)
            
            logging.info(f"Processing {total_patients} patients from {split} set")
...
def main():
    ...
    # Configuration
    DATASET_PATH = "./dicom_dataset"  # Path to DICOM dataset
    OUTPUT_PATH = "./output"  # Output path
    MAX_PATIENTS = 250  # Number of patients to process
    
    # Initialize and run processor
    processor = DatasetProcessor(DATASET_PATH, OUTPUT_PATH, MAX_PATIENTS)
